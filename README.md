...
A pre-built Windows installer can be generated with:
...
This produces `dist/CSV Links Downloader Setup 1.0.0.exe` — a standard NSIS installer that:
...
> **Note:** The first time you run `npm run dist` on Windows, the build tool needs to create symbolic links.
> Enable **Developer Mode** in *Settings → Privacy & Security → For developers*, or run the command once from an **Administrator** PowerShell.
...
├── README.md
└── dist/            # Generated by `npm run dist` (git-ignored)
    ├── CSV Links Downloader Setup 1.0.0.exe   # NSIS installer
...
| Command | Description |
|---|---|
| `npm start` | Launch the app directly from source |
| `npm run dist` | Build the Windows NSIS installer into `dist/` |
| `npm run dist:dir` | Build an unpacked portable directory (faster, no installer) |
